    ...
    <div class="card shadow-sm">
      <div class="card-header bg-transparent border-bottom">
        <div class="row">
          ...
          <?php if(userHasPermission($pdo, $_SESSION["user_id"], 'create_unit')){ ?>
          <div class="col">
      ...
      <div class="card-body">
        <table id="unitTable" class="table table-hover table-cs-color"></table>
      </div>
    </div>
  </div>
</div>
  ...
  $(document).ready(function () {
    var table = $('#unitTable').DataTable({
      ...
      columns: [
        ...
        <?php if(userHasPermission($pdo, $_SESSION["user_id"], 'update_unit') || userHasPermission($pdo, $_SESSION["user_id"], 'delete_unit')){ ?>
        , {
          data: null,
          className: "text-center",
          title: 'Action',
          defaultContent: `
            <?php if(userHasPermission($pdo, $_SESSION["user_id"], 'update_unit')){ ?>
            <button class='btn btn-primary btn-sm btn-edit'><i class='fa-regular fa-pen-to-square'></i></button>&nbsp;
            <?php } ?>
            <?php if(userHasPermission($pdo, $_SESSION["user_id"], 'delete_unit')){ ?>
      ...
      swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            url: 'admin/process/admin_action.php',
            method: 'POST',
            data: { action: 'deleteUnit', unit_id: rowData.unit_id, csrf_token: '<?php echo $_SESSION["csrf_token"]; ?>' },
            dataType: 'json',
            success: function (response) {
              if (response.success) {
                LoadTable();
